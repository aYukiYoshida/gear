...
function mkselection (){
	...
	# gradestd 
	grade=grstd
	selection_criteria="(GRADE==0||GRADE==1||GRADE==2)&&(STATUS>=0&&STATUS<=524287)"
	createXCO
	
	# gradestdmul 
	grade=grstdmul
	selection_criteria="(GRADE==1||GRADE==2)&&(STATUS>=0&&STATUS<=524287)"
	createXCO
}
...
for xisnum in xi0 xi1 xi2 xi3
  do
		#check file
  timpfile=`ls ${datafolder}/xis/event_uf/ae*${xisnum}_?_timp????_uf.evt* 2> /dev/null | tail -1`
  if [ _${timpfile} = _ ]; then
      cat << EOF 1>&2
`basename $0` : timing mode files are not found...
EOF
      continue
  fi
...
#  ls ${datafolder}/xis/event_uf/ae*${xisnum}*uf.evt* > ${evtfilelist}
  ls ${datafolder}/xis/event_uf/ae*${xisnum}_?_timp????_uf.evt* > ${evtfilelist}
...
EOF
  ...
  selection_criteria_std="(GRADE==0||GRADE==1||GRADE==2)&&(STATUS>=0&&STATUS<=524287)" # see Psum's recipe
